...
export function useCustomerManager(
  ...
  const categories = useMemo(
    () => Array.from(new Set(contacts.map((c) => c.customFields?.category || 'General'))),
    [contacts],
  )
          ...
          prev.map((c) =>
            c.id === contact.id
              ? {
                ...
                waId: isRegistered ? `${contact.formattedPhone}@s.whatsapp.net` : undefined,
...
export function useCustomers(contacts: Contact[]) {
  ...
  const filtered = contacts.filter((c) => {
    ...
    if (search && !c.name.toLowerCase().includes(search.toLowerCase()) && !c.rawPhone.includes(search))
      return false
    return true
  })
  ...
  return { search, setSearch, statusFilter, setStatusFilter, filtered, selectedIds, toggleSelect, selectAll, clearSelection, selectedContacts }
}
