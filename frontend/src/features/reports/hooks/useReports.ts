...
export function useReportsManager(campaigns: Campaign[], queue: QueueItem[]) {
  ...
  const [selectedMetric, setSelectedMetric] = useState<'delivery' | 'unregistered' | 'sent'>('delivery')
  ...
  const totals = useMemo(() => {
    ...
    const sentToday = queue.filter((q) => q.status === 'sent' && q.sentAt && q.sentAt >= today).length
