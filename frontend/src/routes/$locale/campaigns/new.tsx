...
function NewCampaignRoute() {
  ...
  const handleLaunchCampaign = async (campaign: Campaign) => {
    await createCampaignAsync({
      ...
      template: { id: '', title: '', category: '', text: campaign.templateText ?? '', suggestedVariables: [], createdAt: Date.now(), updatedAt: Date.now() },
  ...
  return (
    <CampaignWizard
      ...
      onCancel={() => navigate({ to: '/$locale/campaigns', params: { locale } })}
    />
  )
}
